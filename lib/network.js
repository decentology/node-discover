...
function encrypt(str, key) {
  if (key.length > 32) {
    throw new Error("Key must be 32 characters or less");
  }
  const iv = new Buffer.from(randomBytes(16));
  const cipher = createCipheriv(ALGORITHM, key.padEnd(32), iv);
  let crypted = cipher.update(str, INPUT_ENCODING, OUTPUT_ENCODING);
...
function decrypt(str, key) {
  ...
  const IV = new Buffer.from(textParts.shift(), OUTPUT_ENCODING);

  //extract the encrypted text without the IV
  const encryptedText = new Buffer.from(textParts.join(":"), OUTPUT_ENCODING);

  //decipher the string
  const decipher = createDecipheriv(ALGORITHM, key.padEnd(32), IV);
