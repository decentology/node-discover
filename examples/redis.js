...
const cluster = new dEventEmitter({
	broadcastPort: 5550
	, port: 5551
	, key: "something else",
});

let interval = null, hosts = {}, isMaster = false;

let redisPreferredMaster = "192.168.100.17";
...
cluster.on('node::added', function (node) {
	const host = hosts[node.address] = hosts[node.address] || {};
...
const AppConsole = require("appconsole").AppConsole;

const a = new AppConsole();
